...
const descComponents = (isDarkTheme: boolean): PortableTextComponents => ({
  block: {
    normal: ({ children }) => (
      <p className={`text-sm pb-2 leading-relaxed sm:text-lg ${isDarkTheme ? 'text-slate-300' : 'text-slate-500'}`}>
        {children}
      </p>
    ),
  },
  marks: {
    ...
    link: ({ children, value }) => {
      const href: string = toAbsoluteUrl(value?.href) ?? ''
    ...
    >
      ...
      >
        ...
        {(eyebrow || heading || subHeading) && (
          <div className={`mb-12 max-w-5xl mx-auto ${isCentered ? 'mx-auto text-center' : ''}`}>
        ...
        >
          {items.map((item, idx) => (
            ...
            >
              ...
              {item.link?.label && (
                <Link
                  href={toAbsoluteUrl(getFeatureLinkHref(item.link))}
                  prefetch={false}
        ...
        {footerNote && footerNote.length > 0 && (
          <div className='flex items-center justify-center'>
            <div className={`mt-12 inline-flex flex-wrap mx-auto px-6 py-2 rounded-[30px] items-center justify-center gap-2 text-center !text-lg bg-[#13A89E]/10 ${isDark ? 'text-slate-400' : 'text-[#020210]'}`}>
              ...
              <PortableText value={footerNote} components={descComponents(isDark)} />
            </div>
          </div>
        )}
