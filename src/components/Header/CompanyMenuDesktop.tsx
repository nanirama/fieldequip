...
export default function CompanyMenuDesktop({
  ...
  return (
    <li className="group/company relative list-none">
      <button
        type="button"
        aria-haspopup="true"
        className={[
          ...
          <ul className="grid grid-cols-2 gap-x-10 gap-y-6" role="none">
            {menuItems.map((item) => {
              ...
              <li key={`${item.title}-${href}`} className="min-w-0">
                <Link
                  href={href}
                  className="inline-flex items-center gap-2 text-lg font-[700] leading-tight text-[#020210] transition-colors hover:text-teal-600 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-teal-500 focus-visible:ring-offset-2"
                >
                  {item.title}
